subroutine pot(r,v,dv)
  ...
  call V_MLR_Xe_He(r,v,dv,d2v)
  v=v*1.438777356765961d0
  dv=dv*1.438777356765961d0
  return
end subroutine

!***********************************************************************
subroutine V_MLR_Xe_He(R, V, dVdR, d2VdR2)
  ! 
  !  Morse/Long-Range potential energy curve for Xe-He system.
  !  Raw data from:
  !    Liu, Zhai, McCourt, and Li
  !    to be submitted(2021)
  !
  !* On input:
  !    real(8) R:       (Angstrom)
  ...
  !*********************************************************************
  ! Parameters...
  real(8), parameter   :: De = 21.07212D0, Re = 3.970277D0, Rref = 4D0
  integer, parameter   :: p = 5 , q = 3
  integer, parameter   :: N_C = 3 
  integer, parameter   :: m(N_C) = (/ 6, 8, 10 /)
  real(8), parameter   :: C(N_C) = &
    (/   9.41707000D+04,   6.79370000D+05,   5.94000000D+06 /)
  integer, parameter   :: beta_max = 3
  real(8), parameter   :: beta(0:beta_max) = &
    (/  -1.191220000000D-01 ,    9.350000000000D-02 ,    6.690000000000D-02 ,    2.554400000000D-01 /)
  real(8), parameter   :: rho = 1.1D0, s=-0.5D0, bds=3.69D0, cds=0.405D0
  !*********************************************************************
