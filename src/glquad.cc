...
//
{
  ...
  double temp;
  double temp2;
  double zemu = 0.0; // Initialized by yuzhai
...
//
{
  double al = 0.0; // initialized by yuzhai
  double be = 0.0; // initialized by yuzhai
  int i;
  int k;
  int l;
  double p;
  double shft = 0.0; // initialized by yuzhai
  double slp = 0.0; // initialized by yuzhai
  double temp;
  double tmp;
...
void GLIntegrator::reset_workspace(size_t ngrids) {
  clean_workspace();
  ngrids_ = ngrids;
  xs_.resize(ngrids_);
  ws_.resize(ngrids_);
...
std::tuple<double, double, bool> GLIntegrator::integrate(double rtol,
  ...
  std::cerr << "WARNING\n"
            << __FILE__ << ' ' << __LINE__ << ": "
            << "The Gauss-Laguerre Quadrature did NOT meet the errtol "
